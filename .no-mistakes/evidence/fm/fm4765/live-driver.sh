...
# ---- issue #4765 live driver ----
# CONTROL may point at the base-commit fm-control.sh via FM4765_CONTROL.
[ -n "${FM4765_CONTROL:-}" ] && CONTROL=$FM4765_CONTROL
setup_sm() {  # <dir> <id>
  local dir=$1 id=$2 home="$1/home"
  mkdir -p "$home/config"; printf 'claude\n' > "$home/config/secondmate-harness"
  fm_git_worktree "$dir/proj" "$dir/smhome" sm-branch >/dev/null 2>&1
  mkdir -p "$dir/smhome/state" "$dir/smhome/data" "$dir/smhome/bin"
  ...
  printf 'window=fmses:fm-%s\nendpoint_task_id=%s\nworktree=%s\nproject=%s\nharness=claude\nkind=secondmate\nmode=secondmate\nyolo=off\nmodel=default\neffort=default\nhome=%s\nprojects=\n' \
    "$id" "$id" "$dir/smhome" "$dir/smhome" "$dir/smhome" > "$home/state/$id.meta"
  printf 'fm-%s\n' "$id" > "$dir/fake/windows"; printf '%s' "$dir/smhome" > "$dir/fake/cwd"
}
echo "== control script: $CONTROL"
mode=${1:?}
dir=$(new_case live4765 sm9); setup_sm "$dir" sm9
case "$mode" in
  vanish)  # find hits an entry that vanished between readdir and processing (the reported failure)
    real_find=$(command -v find)
    cat > "$dir/fakebin/find" <<SH
#!/usr/bin/env bash
for a in "\$@"; do [ "\$a" = "$dir/smhome/state" ] && { echo "find: \$a/.hash-7: No such file or directory" >&2; exit 1; }; done
exec "$real_find" "\$@"
SH
    chmod +x "$dir/fakebin/find"
    ( while :; do for i in $(seq 1 50); do : > "$dir/smhome/state/.hash-$i"; : > "$dir/smhome/state/.count-$i"; : > "$dir/smhome/state/.last-$i"; done; rm -f "$dir/smhome/state"/.hash-* "$dir/smhome/state"/.count-* "$dir/smhome/state"/.last-*; done ) & churn=$!
    ;;
  churn) ( while :; do for i in $(seq 1 50); do : > "$dir/smhome/state/.hash-$i"; : > "$dir/smhome/state/.last-$i"; done; rm -f "$dir/smhome/state"/.hash-* "$dir/smhome/state"/.last-*; done ) & churn=$! ;;
  unreadable) chmod 000 "$dir/smhome/state/c2.meta" ;;
  metadir) mkdir "$dir/smhome/state/bad.meta" ;;
  unlistable) chmod 000 "$dir/smhome/state" ;;
  nostate) rm -rf "$dir/smhome/state" ;;
esac
echo "== state/ before relaunch:"; ls -la "$dir/smhome/state" 2>&1 | sed 's/^/   /' | head -12
echo "== \$ fm-control.sh sm9 relaunch"
out=$(run_control "$dir" sm9 relaunch); rc=$?
[ -n "${churn:-}" ] && { kill $churn; wait $churn 2>/dev/null; }
chmod 755 "$dir/smhome/state" 2>/dev/null; chmod 644 "$dir/smhome/state/c2.meta" 2>/dev/null
printf '%s\n' "$out" | sed 's/^/   /'
echo "== exit code: $rc"
echo "== secondmate agent command now: $(cat "$dir/fake/command")"
echo "== journal children=: $(journal_field "$dir" sm9 children 2>/dev/null)"
