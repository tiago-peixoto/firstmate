...
. "/Users/tiago/.no-mistakes/worktrees/762e4773438f/01M2JRW88JF1G1JKEDF2CG1EXV/tests/lib.sh"
...
test_herdr_registered_status_over_a_shell_only_pane_is_stale_not_live
test_agy_unregistered_path_without_a_dialog_fails_the_spawn
echo "--- task status file written by fm-spawn.sh agy_spawn_fail:"
cat "$HOME_DIR/state/"agy-nodialog-*.status
. "$ROOT/bin/fm-classify-lib.sh"
l=$(grep '^failed' "$HOME_DIR/state/"agy-nodialog-*.status | tail -1)
e=$(status_line_at_epoch "$l") && echo "emission epoch=$e now=$(date +%s) age=$(( $(date +%s) - e ))s" || echo "UNKNOWN emission time"
