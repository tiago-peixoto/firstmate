...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-live-4228/briefhome/state/wait-scout.status'`
...
Firstmate steers you through durable message files in '/tmp/fm-live-4228/briefhome/state/wait-scout.inbox'.
When a terminal message says an instruction is waiting there, list '/tmp/fm-live-4228/briefhome/state/wait-scout.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-live-4228/briefhome/state/wait-scout.inbox'/NNN.msg '/tmp/fm-live-4228/briefhome/state/wait-scout.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.
Every waiting instruction rings, so never list the inbox on your own.

# Definition of done
Write your findings to `/tmp/fm-live-4228/briefhome/data/wait-scout/report.md`.
The report must stand alone: what you did, what you found, the evidence (commands run, output, file:line references), and what you recommend.
If your deliverable is a visual artifact the captain will review and iterate on, you may host the Lavish review loop yourself (poll, revise, re-serve, staying alive) instead of handing it back to firstmate.
Before reporting done, read and follow `/tmp/fm-live-4228/home/.agents/skills/captain-hold-lifecycle/SKILL.md` and pass its shared completion gate for the report and any visual review.
