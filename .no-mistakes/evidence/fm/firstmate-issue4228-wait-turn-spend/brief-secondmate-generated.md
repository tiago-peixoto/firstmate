...
Nobody reads this chat: the captain and the main firstmate see only what is appended to '/tmp/fm-live-4228/briefhome/state/wait-sm.status', and a captain-facing sentence that is not appended there has not been sent.
...
Firstmate steers you through durable message files in '/tmp/fm-live-4228/briefhome/state/wait-sm.inbox'.
When a terminal message says an instruction is waiting there, list '/tmp/fm-live-4228/briefhome/state/wait-sm.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-live-4228/briefhome/state/wait-sm.inbox'/NNN.msg '/tmp/fm-live-4228/briefhome/state/wait-sm.inbox'/handled/`.
...
Report only true captain-relevant outcomes or a declared external wait by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-live-4228/briefhome/state/wait-sm.status'`
