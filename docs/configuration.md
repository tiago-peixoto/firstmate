...
FM_SECONDMATE_WAKE_STALL_SECS=60   # minimum age of the oldest valid foreign wake-queue row before an endpoint-recorded local secondmate can produce one durable parent wake-loop-stall notification; a latest blocked status always wakes, and only fm_backend_agent_state=alive together with an exact semantic busy verdict stays quiet because every other combination can reflect a stale busy record and silently strand the queue; zero or invalid values use 60
