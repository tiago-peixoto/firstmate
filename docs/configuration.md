...
FM_SECONDMATE_WAKE_STALL_SECS=60   # minimum age of the oldest valid foreign wake-queue row before canonical secondmate current state and recorded-endpoint busy state are checked; current blocked takes precedence and reports stalled, otherwise exact idle or dead reports stalled, exact busy suppresses, and every other evidence combination reports undetermined with its concrete evidence; zero or invalid values use 60
