...
FM_PAUSE_RESURFACE_SECS=3600       # seconds between bounded rechecks of a declared external wait or verified captain-held transfer, including a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting, and both supervisors age that window against the crew's standing declaration rather than against pane busy state or the status log's latest append
FM_SECONDMATE_WAKE_STALL_SECS=60   # minimum idle age of the oldest valid foreign wake-queue row, counted from the later of row arrival and the mate's latest completed turn, before one durable parent wake-loop-stall notification; zero or invalid values use 60; a busy mate is exempt, and a mate whose busy state or turn-end time cannot be read uses a fixed four-hour fallback instead
