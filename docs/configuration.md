...
FM_SECONDMATE_WAKE_STALL_SECS=60   # minimum age of the oldest valid foreign wake-queue row before canonical secondmate current state and recorded-endpoint busy state are checked; current blocked always wakes, exact busy suppresses, current working plus a live endpoint suppresses when semantic busy state is unavailable, and idle, dead, or unavailable evidence wakes conservatively; zero or invalid values use 60
