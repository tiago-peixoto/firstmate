...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge or review activity or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
`bin/fm-pr-check.sh` is the only PR-watch arming path.
It preserves a matching canonical merge poll or publishes a new one, and it separately captures and publishes the initial GitHub review observation.
Canonical merge polls remain on `FM_CHECK_INTERVAL`, while GitHub review, conversation-comment, review-thread-comment, and lookup-health observations run on the existing `FM_POLL` supervision cycle.
Each successful review read advances `state/<id>.pr-poll-seen`; a failed read changes its health to `error` and wakes once, so an unreadable review monitor cannot look like a quiet one.
Recovery is also visible, and a new review with no requested reviewer carries the `NOBODY requested` warning.
When a canonical validated merge poll returns exactly `merged`, the watcher appends that durable notification before publishing a private receipt bound to the poll's registration, bytes, file identities, metadata, provider, URL, and task ID.
