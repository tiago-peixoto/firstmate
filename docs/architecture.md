...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR movement polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, except a wait on a still-open pull request that a live movement poll demonstrably covers, and heartbeat backstop hits.
...
An exact semantic busy verdict exempts that row, while an idle mate reaches `FM_SECONDMATE_WAKE_STALL_SECS` from the later of row arrival and the settled time `bin/fm-busy-lib.sh` reports for its idle verdict before the primary appends one keyed `check` wake naming the mate, row sequence, and observed idle age.
A dead or missing endpoint, an unknown verdict, or an idle verdict with no settled time retains the alarm behind a fixed four-hour fallback, which covers one long turn without adding another operator setting.
Parent receipts and queued-key deduplication suppress repeats for the same row across watcher and handling crashes, while empty and younger queues remain silent.
...
`tests/fm-wake-queue.test.sh` pins the notification, busy-exemption, idle-clock, four-hour fallback, idempotence, quiet-queue, and byte-for-byte foreign-row preservation guarantees.
...
In that status-log fallback, a declared external wait reports the distinct `paused` state with its reason, read as the standing declaration described below rather than as the log's last line.
