...
An exact semantic busy verdict exempts that row, while a confirmed idle mate reaches `FM_SECONDMATE_WAKE_STALL_SECS` from the later of row arrival and its latest completed turn before the primary appends one keyed `check` wake naming the mate, row sequence, and observed idle age.
An unavailable busy verdict retains the alarm after a fixed four-hour fallback, which covers one long turn without adding another operator setting.
