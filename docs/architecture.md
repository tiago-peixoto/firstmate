...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR review activity or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Registered mates without usable local secondmate metadata remain outside this scan because startup secondmate-liveness owns missing endpoint recovery, and remote homes retain their host-local supervision boundary.
When local metadata and a foreign queue exist but the recorded target is empty, the aged row produces the keyed `undetermined` check with `endpoint-busy=unknown no-target` evidence.
`tests/fm-wake-queue.test.sh` pins the busy, blocked, idle, undetermined, idempotence, quiet-queue, and byte-for-byte foreign-row preservation guarantees.
`bin/fm-pr-check.sh` is the only PR-monitor arming path. It captures the initial observation and publishes one canonical monitor for GitHub reviews, conversation comments, review-thread comments, lookup health, and merge; GitLab retains merge and lookup-health coverage in that same mechanism. Canonical PR monitors run on the existing `FM_POLL` supervision cycle, independently of `FM_CHECK_INTERVAL`, while unchanged observations remain silent. Each successful read advances `state/<id>.pr-poll-seen`; a failed read changes its health to `error` and wakes once, so an unreadable monitor cannot look like a quiet one. Recovery is also visible, and a new review with no requested reviewer carries the `NOBODY requested` warning.
When a canonical validated PR monitor returns exactly `merged`, the watcher routes it through the shared merge-outcome emitter before retiring the monitor.
...
After successful outcome publication, the watcher immediately delivers the emitter's local actionable row and publishes a private receipt bound to the monitor's registration, bytes, file identities, metadata, provider, URL, and task ID.
The receipt makes monitor cleanup safely retryable across restarts: fixed-path recovery revalidates the same evidence, removes the runnable check first, removes its registration and data sidecars, removes the receipt last, and preserves task metadata including `pr=` and `pr_head=`.
...
A `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh pr merge <n> --repo <owner>/<repo>`, defaults to `--squash`, and preserves the caller's own explicit merge-method flag in either its long or short form.
