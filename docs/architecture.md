...
Once that row reaches `FM_SECONDMATE_WAKE_STALL_SECS`, the primary reads the mate's own last status verb and live semantic busy verdict.
A last `blocked:` verb always produces one keyed `check` wake naming the mate, row sequence, and observed age, while an exact busy verdict suppresses it regardless of row age; idle, dead, and unknown liveness produce the wake conservatively so missing evidence cannot hide a mate waiting on the primary.
