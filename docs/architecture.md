...
Once that row reaches `FM_SECONDMATE_WAKE_STALL_SECS`, a latest `blocked:` status always wakes the primary, and only the conjunction of `fm_backend_agent_state=alive` with an exact semantic busy verdict keeps the watcher quiet for however long the turn legitimately runs, because without positive agent liveness a cached busy verdict may be stale and silently strand the queue; every other agent-state and busy-verdict combination conservatively wakes, the primary appends one keyed `check` wake naming the mate, row sequence, and observed age, parent receipts and queued-key deduplication suppress repeats for the same row across watcher and handling crashes, and empty or younger queues remain silent.
Registered mates with no recorded endpoint remain outside this scan because startup secondmate-liveness owns missing-endpoint recovery, while a recorded endpoint whose agent is dead remains inside the scan and wakes; remote homes retain their host-local supervision boundary.
...
Because status files remain unframed append-only streams, state consumers select the latest recognized state event separately from signal and heartbeat consumers selecting legacy captain-relevant lines with case-insensitive POSIX ERE matching; relevant continuation prose can still surface without displacing the recognized state used for blocked and declared-wait decisions, and configured pause, resolve, and captain-held verbs apply to both selectors.
