...
| `fm-fork-lib.sh`         | Single owner of every Git fact more than one fork script reads, so no two of them can drift apart |
...
| `fm-pr-lib.sh`           | Own canonical task and PR validation plus private atomic merge-poll publication, review observation health, and identity-bound retirement |
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated PR/MR merge-poll sidecars     |
| `fm-pr-review-poll.sh`   | Read validated GitHub PR review, comment, and lookup-health observations             |
| `fm-pr-check-migrate.sh` | Quarantine older task polls without execution and rebuild only canonical polls       |
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, preserve or arm the merge poll, and add GitHub review coverage |
