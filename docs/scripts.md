...
| `fm-pr-review.sh`        | Operate Open Sourcerer Monitor, inspect routed changes, guard review writes, deliver external replies, and control per-PR coverage |
| `fm-pr-review-state.mjs` | Own bounded PR transition comparison, exact-head queue, generation lane, routing hints, private findings, and replay-safe responses |
