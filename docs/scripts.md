...
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated pull-request movement, merge-request, and Gerrit-change poll sidecars |
...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static movement poll, seed its GitHub baseline reading, and register contribution observation; refuses a GitHub draft |
