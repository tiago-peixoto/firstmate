...
`docs/sessionstart-nudge.md` covers the silent session-open hook use, while `fm-gate-refuse-lib.sh`'s header owns the retained legacy validation-agent containment check.
...
| `fm-gate-refuse-lib.sh`  | Retained legacy validation-agent containment for fleet lifecycle entrypoints           |
...
| `fm-nm-run-lib.sh`       | Legacy branch-and-code-identity attribution for already-running retired-mode tasks  |
