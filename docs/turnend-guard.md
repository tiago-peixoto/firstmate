...
`bin/fm-watch.sh` refreshes `state/.last-watcher-beat` between the slow phases of a cycle and immediately before its terminal wait (`event_wait_or_sleep`), so a healthy watcher's beacon ages by at most one phase, or `FM_POLL` seconds during that wait.
A phase that hangs past the grace still reads stale, because the beacon is not refreshed while a phase is running and nothing ticks it from the background.
