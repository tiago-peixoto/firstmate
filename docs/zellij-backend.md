...
Spawn seats a leased Treehouse copy with a top-level `cd`, then confirms the pane cwd against that exact path.
This active probe is scoped to spawn-time seating and is not advertised as a general live-cwd API.
