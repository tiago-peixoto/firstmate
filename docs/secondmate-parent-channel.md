...
|---|---|---|
| Ship child PR ready | the child's ready `done` line naming the PR URL; `pr=` in the child's record once registered | `bin/fm-inactive-reconcile.sh` on the next poll with the child's line; `bin/fm-pr-check.sh` at registration with the canonical URL |
