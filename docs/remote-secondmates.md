...
A Claude second mate launches on the host's default Claude login, with `CLAUDE_CONFIG_DIR` unset.
A Pi second mate launches on the supervisor account pinned in the code root's `config/pi-agent-dir`, and is held to that code root's `config/pi-account-side`, never to the remote home's own worker pin or side; [Account pins](configuration.md#account-pins) owns the contract.
