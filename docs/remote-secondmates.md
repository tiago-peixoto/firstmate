...
A Claude or Pi second mate launches on the supervisor account pinned in the code root's `config/claude-config-dir` or `config/pi-agent-dir`, and a Pi one is held to that code root's `config/pi-account-side`, never to the remote home's own worker pins or side; [Account pins](configuration.md#account-pins) owns the contract.
