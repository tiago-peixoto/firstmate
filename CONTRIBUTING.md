...
3. Run the focused tests, lint, and documentation checks described below.
4. Commit your changes and push your feature branch to your fork:

   ```sh
   git push <fork-remote> HEAD
   ```

5. Open a pull request against the parent repository and include the behavior and verification evidence reviewers need.
...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, and pinned shellcheck version) that CI runs.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through a pull request on a feature branch and require an explicit merge approval.
...
bin/fm-lint.sh   # lint that same surface; the single owner CI runs, full set in CI
...
bin/fm-test-run.sh --all   # deliberate complete regression
