...
`config/pi-agent-dir` is not inherited either, because it selects that home's own Pi account root; provision it directly in every secondmate home that launches Pi, before its first launch, because a home without one refuses Pi launches (`docs/configuration.md` "Account pins").
