...
`config/pi-agent-dir` and `config/pi-account-side` are not inherited either, because each names the root or side that home's own workers and scouts launch on; provision each directly in every secondmate home whose workers use Pi, because a home without the pin refuses those launches, while the second mate's own launch runs on the launching home's supervisor account and side (`docs/configuration.md` "Account pins").
Claude is not pinned: every Claude launch unsets `CLAUDE_CONFIG_DIR` and uses the default login, and a leftover `config/claude-config-dir` is ignored.
A secondmate home that leaves `config/pi-account-side` absent is personal-side and cannot reach the work account, so a work-domain secondmate has to declare `work` itself.
