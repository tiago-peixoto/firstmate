...
`config/claude-config-dir`, `config/pi-agent-dir`, and `config/pi-account-side` are not inherited either, because each names the account, root, or side that home's own workers and scouts launch on; provision each directly in every secondmate home whose workers use that runner, because a home without the pin refuses those launches, while the second mate's own launch runs on the launching home's supervisor account and side (`docs/configuration.md` "Account pins").
