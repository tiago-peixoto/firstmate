...
- `direct-PR` pushes and opens a PR.
...
Never migrate or reinterpret an existing unannotated or retired registry value silently.
Stop and obtain an explicit choice of `direct-PR` or `local-only` for that project before dispatch or provisioning.
