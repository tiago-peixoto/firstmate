...
| Attribution | Cursor injects a Co-Authored-By / Made-with trailer at the tooling layer after the typed message. `fm-spawn` does not rely on `~/.cursor/cli-config.json`; it installs a pane-scoped git `commit-msg` strip (`bin/fm-git-strip-ai-trailers.sh`). |
