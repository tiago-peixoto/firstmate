...
| Attribution | Cursor injects a Co-Authored-By / Made-with trailer at the tooling layer after the typed message. `fm-spawn` does not rely on `~/.cursor/cli-config.json`; it installs a pane-scoped git `commit-msg` strip (`bin/fm-git-strip-ai-trailers.sh`), which removes AI `Co-Authored-By` trailers and a line beginning with the generated-with form. A `Made with <product>` line is not matched, and `git commit --no-verify` skips the hook, so a trailer on a Cursor commit means one of those two, not a matcher gap. `attribution.attributeCommitsToAgent` defaults to on when unset and only feeds the request to the server, which is why it suppresses the trailer rather than preventing it; verified 2026-09-17 on 2026.09.15-d2fe57e that with it on the agent's own `git commit` puts `Co-authored-by: Cursor <cursoragent@cursor.com>` into `.git/COMMIT_EDITMSG`, where the strip removes it. |
