...
Public X relay is inert until the home has the configured pairing token.
That token authorizes normal reversible public replies, not destructive, irreversible, or security-sensitive action.
Load `fmx-respond` on an X mention, X configuration error, promised-public-followup event, or any milestone or terminal event for X-linked work.
...
X-only operation still requires live supervision.
