...
# addressed through the native gh binary by the derived owner and repository; a GitLab merge
...
# supplies that queue-aware read, and gh is the only reader: the node-launched
# wrapper that used to stand in as a degradation path is retired, so an absent
# gh and a failed gh read each refuse rather than accepting an unproven outcome.
...
# view; it is the only reader, so an outcome it cannot prove is refused.
FM_PR_GITHUB_STATE=
FM_PR_GITHUB_MERGED=
FM_PR_GITHUB_QUEUED=
FM_PR_GITHUB_BASE=
...
github_read_outcome() {
  # gh is the ONLY reader. The retired node-launched wrapper used to stand in as
  # a degradation path that could prove a landed merge without gh; with it gone
  # there is no second view, so an unreadable outcome refuses rather than
  # guessing. A gh read that completes and reports the pull request as neither
  # merged nor queued is a concrete outcome, not a missing one, and keeps its
  # own refusal.
  if ! command -v gh >/dev/null 2>&1; then
    echo "error: could not read the GitHub pull request outcome after the merge attempt: gh is not on PATH and there is no second reader; PR metadata and merge poll remain recorded" >&2
    return 1
  fi
  github_read_outcome_with_gh && return 0
  echo "error: could not read the GitHub pull request outcome after the merge attempt: the gh read failed; PR metadata and merge poll remain recorded" >&2
  return 1
}
