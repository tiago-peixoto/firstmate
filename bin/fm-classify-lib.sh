...
status_standing_wait_line() {  # <status-file> -> the standing declaration line, or empty
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    if _fm_verb_is_declared_wait "$verb"; then
      ...
      standing=''
    elif [ -n "$standing" ] && [ "$verb" = "$resolve" ]; then
      # A pause is retracted only by a resolution for its own phase key, where a
      # keyless line is its own phase distinct from a stated [key=default]
      # (status_declared_wait_line above owns that rule). A captain hold is
      # retracted by any resolution: the hold waits on firstmate's answer, and
      # upstream's read never lets a hold outlive a later resolved line.
      if status_is_captain_held "$standing" \
        || [ "$(_fm_decision_key "$line" "$_FM_CLASSIFY_KEYLESS_PHASE" || printf '%s' "$_FM_CLASSIFY_KEYLESS_PHASE")" \
           = "$(_fm_decision_key "$standing" "$_FM_CLASSIFY_KEYLESS_PHASE" || printf '%s' "$_FM_CLASSIFY_KEYLESS_PHASE")" ]; then
        standing=''
      fi
    fi
  done < "$f"
