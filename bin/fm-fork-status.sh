...
# dispositions with gh. Without it, the report is network-free and uses local
# refs plus recorded dispositions. gh's --jq formatter emits the selected live
# disposition as one raw non-empty scalar.
...
if [ "$REFRESH" -eq 1 ]; then
  while IFS=$'\t' read -r id url recorded; do
    ...
    live=$(gh api "/repos/$owner/$repo_name/pulls/$number" \
      --jq 'if .merged_at != null then "merged" elif .state == "open" then "open" else "closed" end' 2>/dev/null || true)
    case "$live" in
      open|closed|merged) ;;
      *) add_error "manifest unit $id pull request disposition could not be refreshed through gh"; continue ;;
    esac
