...
#     ...                        a pre-field v1 record reads as an empty list)#   confirmed: <UTC ISO 8601>
...
#       [--action <verb> --object <text> --when <text> [--stop <text>]]...
#       [--expected-return <UTC ISO 8601>] [--spend <n>] [--grant <task-id>]...#     Compile and write the proposal, then print the read-back. Exit 0 with every
...
#     exists; invalid or duplicate ids are a usage error, never a refused clause.#   fm-afk-contract.sh confirm
...
#   fm-afk-contract.sh grants [--proposal | --path <record>]    one task id per line#   fm-afk-contract.sh archive              move the record aside; print its path
...
fm_afk_contract_main() {
  ...
  case "$cmd" in
    ...
    grants)
      path=$(fm_afk_contract_select_path "$@") || { fm_afk_contract_usage >&2; return 2; }
      [ -f "$path" ] || { fm_afk_contract_log "no record at $path"; return 1; }
      fm_afk_contract_read_grants "$path" ;;    archive) fm_afk_contract_cmd_archive ;;
    archived)
