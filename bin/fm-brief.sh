...
Use \`$PAUSED_VERB: {why}\` (distinct from \`blocked:\`) only when your domain is deliberately idling on a known external wait you expect to clear on its own; that declaration stands until you retract it with \`$RESOLVE_VERB: {how it cleared}\` (or end the task with \`done:\`/\`failed:\`/\`blocked:\`/\`needs-decision:\`); a later \`working:\` line does not cancel it. Use \`blocked:\` when you are stuck and need firstmate to act.
...
4. Report status by appending one line:
   ...
   treating it as a possible wedge. That declaration STANDS until you retract it
