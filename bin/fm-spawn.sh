...
#   the explicit mode differs from the project's supported standing posture, a
#   loud one-line deviation notice is printed and the spawn continues.
#   Retired or malformed registry values stop a fresh spawn for explicit migration.
  ...
  if [ "$KIND" = ship ]; then
    ...
    case "$MODE" in
      ...
      no-mistakes|no-mistakes-prod-only)
        echo "error: delivery mode $MODE is retired; choose direct-PR or local-only" >&2
        exit 1 ;;
...
if [ "$KIND" = ship ] && [ "$RELAUNCH" -eq 0 ]; then
  ...
  # The registry holds the captain's standing posture. A current explicit
  # instruction can choose the other supported path, but the deviation is never
  # silent. Unsafe registry entries stop before any endpoint is created.
  if ! STANDING_LINE=$("$FM_ROOT/bin/fm-project-mode.sh" --raw "$PROJ_NAME" 2>&1); then
    echo "error: cannot launch $ID until the standing posture for $PROJ_NAME is safe: $STANDING_LINE" >&2
    exit 1
  fi
  STANDING_MODE=${STANDING_LINE%% *}
  if [ "$STANDING_MODE" != "$MODE" ]; then
    echo "notice: $ID ships mode=$MODE while the standing posture for $PROJ_NAME is $STANDING_MODE - this differs from the captain's standing posture; proceed only on a current explicit captain instruction or an intake judgment you can state" >&2
  fi
fi
