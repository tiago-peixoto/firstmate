...
# change poll sidecar.
# It reports what actually moved on a GitHub pull request and stays silent
# otherwise, including on every error, so a failed lookup can never be read as
# movement. The provider-tagged identity is data in the sidecar and is never
...
#                   every non-merged GitHub reading. It is a FINGERPRINT, not an
#                   event: this program is a pure function of the pull request
#                   and keeps no memory between runs, so an unchanged reading
#                   prints the same line every poll. Deciding what changed, and
#                   waking only then, belongs to the caller holding the previous
...
# glab's rendered layout, and a Gerrit change reports its merge alone, so
# fm_pr_poll_covers_wait refuses to silence a timed recheck for either.
set -u
LC_ALL=C
export LC_ALL
