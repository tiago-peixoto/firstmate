...
fm_lock_try_acquire() {
  ...
  # Stock macOS Bash 3.2 has no BASHPID and keeps $$ unchanged in a subshell, so
  # BASH_SUBSHELL must also prove this is the original holding frame; without it
  # a subshell reads as the holder and deletes a lock that is still live.
  pid=$(cat "$lockdir/pid" 2>/dev/null || true)
