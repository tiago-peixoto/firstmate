...
One drive call blocks until the next gate or outcome, and one fix round can take around thirty minutes, with up to three rounds chaining.
Hold that wait inside the drive call itself, bounded as the Waiting section says, and when its \`--wait\` elapses re-run the same \`axi run\` or \`axi respond\` to reattach.
This replaces the no-mistakes skill's advice to background the call and check \`no-mistakes axi status\` from a separate call: every such check is a model turn spent only on waiting.
