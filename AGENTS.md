...
config/claude-config-dir  Claude account pin, required for claude launches from this home; LOCAL, gitignored, primary-authoritative, and inherited by secondmate homes; see docs/configuration.md "Account pins"
config/pi-agent-dir  Pi account pin, required for pi and pi-signed launches from this home; LOCAL, gitignored, and deliberately NOT inherited, so every home launching Pi is configured before its first launch; it also selects Pi's project-trust store, so the first launch of a project under a newly pinned root shows Pi's trust dialog; see docs/configuration.md "Account pins"
