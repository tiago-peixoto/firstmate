...
config/claude-config-dir  optional Claude configuration root for launches from this home; LOCAL, gitignored, primary-authoritative, and inherited by secondmate homes; see docs/configuration.md "Claude configuration root"
config/pi-agent-dir  optional Pi configuration/account root for pi and pi-signed launches from this home; LOCAL, gitignored, and deliberately NOT inherited, so each home needing a pin is configured before its first launch; it also selects Pi's project-trust store, so the first launch of a project under a newly pinned root shows Pi's trust dialog; see docs/configuration.md "Pi configuration root"
