...
config/claude-config-dir  Claude account pin: the account this home's claude workers and scouts launch on, and the fallback supervisor account for second mates it launches; LOCAL, gitignored, and deliberately NOT inherited; see docs/configuration.md "Account pins"
config/pi-agent-dir  Pi configuration root for this home's pi and pi-signed workers and scouts, and for Pi second mates it launches; LOCAL, gitignored, and deliberately NOT inherited; it selects the root, while the launch model's provider selects the account inside it; it also selects Pi's project-trust store, so the first launch of a project under a newly pinned root shows Pi's trust dialog; see docs/configuration.md "Account pins"
config/pi-account-side  work/personal side of this home's Pi launches; a launch whose model names a provider the side does not allow, or names none at all, is refused; LOCAL, gitignored, not inherited, and absent means personal; see docs/configuration.md "Pi account side"
config/pi-mcp-config  one absolute path to an MCP config file this home's Pi launches receive as --mcp-config; a named file that is missing refuses the launch rather than falling back to the project's own servers; LOCAL, gitignored, not inherited; see docs/configuration.md "Pi MCP overlay"
...
Firstmate alone resolves a matched profile array: begin with `quota-axi`'s default TOON, read once per account pin at that intake, using the skill's narrow TOON-then-`--json` fallback only for genuine ambiguity, evaluate every configured candidate against its pin's current output, and choose with inspectable `spendPriority` as the one quota-perspective ranker after the skill's eligibility, reasoning-class, and runway-feasibility gates.
