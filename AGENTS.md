...
The ship brief has the worker publish the builder's model family beside the branch on the forge, so never relay it out of task metadata: any home establishes it for an exact head with `bin/fm-pr-provenance.sh show`, which refuses rather than guessing.
