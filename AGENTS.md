...
config/pi-agent-dir  Pi account pin: the account this home's pi and pi-signed workers and scouts launch on, and the supervisor account for Pi second mates it launches; LOCAL, gitignored, and deliberately NOT inherited; it also selects Pi's project-trust store, so the first launch of a project under a newly pinned root shows Pi's trust dialog; see docs/configuration.md "Account pins"
