...
A declared `paused:` is a standing declaration of a bounded external wait expected to clear on its own, retracted only by that worker's own later terminal line, while `blocked:` means firstmate action is needed.
