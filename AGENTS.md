...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records `pr=` and the forge's `pr_head=` when available in the task's meta, preserves or arms the existing merge poll, and automatically adds GitHub review, comment, and lookup-health coverage.
