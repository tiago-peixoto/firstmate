...
#
# Liveness may no longer SUPPRESS a declared pause, so every verdict absorbs here.
# It is still read to recover paused classification when a confidently dead crew's
# state has fallen back to stopped or unknown; removing that promotion would surface
# an exited crew as bare stopped instead of preserving its bounded annotated recheck.
# The worker's declaration remains evidence, with PAUSE_RESURFACE_SECS as the backstop.
#
...
test_exited_declared_pause_is_bounded_and_live_gate_absorbs() {
  ...
  [ -s "$state/.wake-queue" ] \
    || fail "dead-agent declared pause did not produce its bounded paused recheck"
