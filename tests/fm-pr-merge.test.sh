...
# gh binds -s, -m, and -r to its three merge methods and expands a single-dash
# cluster one character at a time, so a caller who picks the method in short form
# must not also receive this script's default --squash. The retired wrapper's
# `--method=<value>` form has no gh equivalent, so it is no longer treated as a
# chosen method: the default is added and gh reports the unknown flag itself,
# which is the visible refusal a silent fall-through would have hidden.
test_short_merge_method_is_not_overridden() {
  ...
  pass "fm-pr-merge treats gh's short and clustered merge-method flags as the caller's choice"
}
...
test_github_still_forwards_head_binding_arg() {
  ...
  # which is the GitLab path. GitHub's extra args stay the caller's business, and
  # gh spells this one --match-head-commit (gh pr merge --help, gh 2.97.0).
