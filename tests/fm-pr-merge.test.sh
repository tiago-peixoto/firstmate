...
#   (al) a failed gh read falls back to the gh view, which can prove a merge
...
#   (aw) a refusal built on the gh view says the merge queue could not be
...
# gh mock recording every invocation to a log file, and gh mock answering
# headRefOid for fm-pr-check.sh's pr_head lookup. Args: case_dir head_sha
add_gh_mocks() {
  local case_dir=$1 head=$2
  # One gh fake, because gh is now the only GitHub tool: it answers the merge,
  # fm-pr-check.sh's headRefOid lookup, the queue-aware graphql outcome read,
  # and the REST rules read. Order matters - the headRefOid form of `pr view`
  # is matched before the --repo outcome form.
...
printf '%s\n' "\$*" >> "\$FM_TEST_GH_LOG"
case "\${1:-} \${2:-}" in
  "pr merge") echo "will be added to the merge queue when all requirements are met" ; exit 0 ;;
  "pr view")
    case " \$* " in
      *headRefOid*) printf '%s\n' '$head' ; exit 0 ;;
    esac
...
SH
  ...
  assert_grep 'pr merge 62 --repo example/repo --squash' "$case_dir/gh.log" \
...
test_github_unreadable_outcome_keeps_pr_bookkeeping() {
  ...
  # There is only one reader now: the retired wrapper's degradation view is gone,
  # so the refusal names the single failed gh read rather than two.
  assert_grep 'the gh read failed' \
    "$case_dir/stderr" "github-outcome-read-fails: the refusal did not name the failed gh read"
  assert_no_grep 'verified: ' "$case_dir/stdout" \
...
test_github_auto_merge_without_queue_refuses_legibly() {
  ...
  for spelling in --auto --auto=true; do
    ...
    [ "$(grep -c '^pr merge ' "$case_dir/gh.log" | tr -d '[:space:]')" = 1 ] \
...
test_github_queue_required_refusal_names_retry_flags() {
  ...
  grep -qxF 'pr merge 54 --repo example/repo --squash' "$case_dir/gh.log" \
...
test_bundled_repo_override_args_refuse_before_recording() {
  ...
  grep -qxF 'pr merge 8 --repo example/repo --squash -d' "$case_dir/gh.log" \
