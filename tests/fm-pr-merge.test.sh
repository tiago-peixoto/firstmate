...
# gh mock recording merge invocations to a log file and answering headRefOid
...
# gh mock that fails the merge call but succeeds everything else, so a real
...
test_method_equals_merge_method_not_overridden() {
  ...
  grep -qxF 'pr merge 23 --repo example/repo --merge' "$case_dir/gh.log" \
    || fail "method-equals-merge-method: legacy --method=merge was not normalized to gh's native --merge flag"
  pass "fm-pr-merge normalizes legacy --method=<value> input to gh's native merge method"
}
