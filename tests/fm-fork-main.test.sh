...
# gh --jq emits the selected scalar directly.
# Refresh accepts that raw non-empty value and rejects silent empty output.
test_refresh_parses_native_gh_scalar() {
  local w repo fakebin out rc log
  w=$(new_world refresh-native-gh)
...
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$FM_TEST_GH_LOG"
[ "${FM_TEST_GH_MODE:-open}" = empty ] || printf '%s\n' open
SH
  chmod +x "$fakebin/gh"
  : > "$log"
  out=$(FM_TEST_GH_LOG="$log" PATH="$fakebin:$PATH" "$STATUS" --repo "$repo" --refresh 2>&1) \
    || fail "refresh rejected gh's native scalar output: $out"
  assert_grep 'api /repos/example/firstmate/pulls/1 --jq ' "$log" \
    "refresh did not select the live disposition through gh --jq"
  assert_contains "$out" 'errors=0' "native gh scalar output created a refresh error"

  set +e
  out=$(FM_TEST_GH_MODE=empty FM_TEST_GH_LOG="$log" PATH="$fakebin:$PATH" \
    "$STATUS" --repo "$repo" --refresh 2>&1)
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "refresh accepted empty gh output"
  assert_contains "$out" 'could not be refreshed through gh' \
    "refresh did not report empty gh output"
  pass "fork health refresh parses native gh --jq output and rejects empty values"
}
