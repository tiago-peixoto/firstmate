...
SH
  ...
  # common "no GitHub PR" baseline. Tests that need a merged PR or a lookup error
...
case "\${1:-} \${2:-}" in
  "pr list")
    case " \$* " in
      *" --json number --jq .[0].number // empty "*) printf '%s\n' 7 ; exit 0 ;;
    esac
    ;;
  "pr view")
    case " \$* " in
...
test_no_pr_recorded_discovers_merged_pr_by_branch_allows() {
  ...
  grep -qxF 'pr list --state all --head fm/task-x1 --limit 1 --json number --jq .[0].number // empty' "$case_dir/gh.log" \
    || fail "no-pr-branch-discovery: gh did not select the first PR number through its own JSON formatter"
  pass "teardown discovers PR number 7 through gh --jq and tears down when no pr= was ever recorded"
}
