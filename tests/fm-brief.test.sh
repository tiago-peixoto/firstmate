...
# metacharacter corruption for every supported ship delivery mode. This also guards
...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    ...
    assert_no_grep "no-mistakes" "$brief" \
      "$id: worker brief still carries the retired delivery pipeline contract"
    assert_no_grep "EOF" "$brief" "$id: brief leaked a heredoc EOF marker (unterminated heredoc)"
  done
  pass "fm-brief.sh: direct-PR/local-only briefs generate cleanly without the retired pipeline contract"
}

# A ship task's delivery mode is firstmate's per-task decision, so a missing or
# unusable value must stop the scaffold instead of silently defaulting. The
# retired rows must direct the caller to choose one of the supported modes.
...
retired pipeline mode|--mode no-mistakes|retired; choose direct-PR or local-only
retired conditional policy|--mode no-mistakes-prod-only|retired; choose direct-PR or local-only
ROWS
...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  assert_no_grep "no-mistakes" "$home/data/$id/brief.md" \
    "local-only brief retained the retired delivery pipeline contract"
  id="brief-direct-intent-a4"
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" direct-proj --mode direct-PR >/dev/null 2>&1
  assert_no_grep "no-mistakes" "$home/data/$id/brief.md" \
    "direct-PR brief retained the retired delivery pipeline contract"
  pass "fm-brief.sh: faster paths use configured authority without stacked review"
}

# A worker receives the generated brief, so every scaffold must exclude the
# retired pipeline contract rather than merely selecting a different branch.
test_worker_briefs_exclude_retired_pipeline_contract() {
  local home id brief kind
  home="$TMP_ROOT/retired-contract-home"
  mkdir -p "$home/data"
  for kind in direct local scout secondmate; do
    id="brief-retired-$kind"
    case "$kind" in
      direct) FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" alpha --mode direct-PR >/dev/null 2>&1 ;;
      local) FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" alpha --mode local-only >/dev/null 2>&1 ;;
      scout) FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" alpha --scout >/dev/null 2>&1 ;;
      secondmate)
        FM_HOME="$home" FM_SECONDMATE_CHARTER='alpha operations' \
          "$ROOT/bin/fm-brief.sh" "$id" --secondmate --no-projects >/dev/null 2>&1 ;;
    esac
    brief="$home/data/$id/brief.md"
    assert_present "$brief" "$kind brief was not scaffolded"
    assert_no_grep "no-mistakes" "$brief" \
      "$kind brief still injects the retired delivery pipeline contract"
  done
  pass "fm-brief.sh: no generated worker instruction carries the retired pipeline contract"
}
