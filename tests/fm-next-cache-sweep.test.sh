...
# --- sweep: it reports a genuinely unowned copy ------------------------------

test_sweep_reports_available_copy_without_deleting() {
  local case_dir wt out rc
  case_dir=$(make_case report-only)
  ...
  expect_code 0 "$rc" "report-only: sweep should succeed"
  assert_present "$wt/packages/frontend/.next" \
    "report-only: pool ownership is not proven, so build output must survive"
  assert_present "$wt/packages/frontend/next.config.ts" "report-only: source must survive"
  assert_present "$wt/README.md" "report-only: tracked content must survive"
  assert_contains "$out" "report-only" \
    "report-only: sweep must distinguish the non-deleting path"
  assert_contains "$out" "$wt/packages/frontend/.next" \
    "report-only: report must name the directory"
...
test_sweep_counts_owned_copy_without_build_output() {
  local case_dir wt out rc
  case_dir=$(make_case owned-empty)
  install_treehouse_stub "$case_dir"
  wt=$(add_pool_worktree "$case_dir" 1)
  printf '1 in-use\n' > "$case_dir/pool-status"
  ...
  expect_code 0 "$rc" "owned-empty: report should succeed"
  assert_contains "$out" "1 copy" \
  ...
  assert_present "$wt" "owned-empty: reporting must leave the owned copy intact"
  pass "an owned copy without build output is counted"
}
...
test_sweep_refuses_undecodable_package_json() {
  local case_dir wt app out rc
  case_dir=$(make_case undecodable-package-json)
  install_treehouse_stub "$case_dir"
  wt=$(add_pool_worktree "$case_dir" 1)
  app="$wt/packages/frontend"
  mkdir -p "$app/.next"
  printf '{"metadata":{"next":true}\n' > "$app/package.json"
  printf 'build\n' > "$app/.next/BUILD_ID"
  printf 'packages/frontend/.next\n' >> "$wt/.gitignore"
  git -C "$wt" add -A >/dev/null 2>&1
  git -C "$wt" -c commit.gpgsign=false -c user.email=t@t -c user.name=t \
    commit -qm "undecodable package json"
  printf '1 available\n' > "$case_dir/pool-status"
  ...
  expect_code 1 "$rc" "undecodable-package-json: invalid JSON must be incomplete"
  assert_present "$app/.next" \
    "undecodable-package-json: invalid JSON must prevent deletion"
  assert_contains "$out" "$wt" \
    "undecodable-package-json: the incompletely inspected copy must be named"
  pass "an undecodable package.json refuses the project"
}

test_sweep_refuses_malformed_later_dependency_table() {
  local case_dir wt app out rc
  case_dir=$(make_case malformed-later-dependency-table)
  install_treehouse_stub "$case_dir"
  wt=$(add_pool_worktree "$case_dir" 1)
  app="$wt/packages/frontend"
  mkdir -p "$app/.next"
  printf '%s\n' \
    '{"dependencies":{"next":"16"},"devDependencies":null}' \
    > "$app/package.json"
  printf 'build\n' > "$app/.next/BUILD_ID"
  printf 'packages/frontend/.next\n' >> "$wt/.gitignore"
  git -C "$wt" add -A >/dev/null 2>&1
  git -C "$wt" -c commit.gpgsign=false -c user.email=t@t -c user.name=t \
    commit -qm "malformed later dependency table"
  printf '1 available\n' > "$case_dir/pool-status"
  ...
  expect_code 1 "$rc" \
    "malformed-later-dependency-table: every dependency table must be valid"
  assert_present "$app/.next" \
    "malformed-later-dependency-table: malformed metadata must refuse deletion"
  assert_contains "$out" "$wt" \
    "malformed-later-dependency-table: the indeterminate copy must be named"
  pass "a malformed later dependency table refuses the project"
}

test_sweep_refuses_nonstandard_json_constant() {
  local case_dir wt app out rc
  case_dir=$(make_case nonstandard-json-constant)
  install_treehouse_stub "$case_dir"
  wt=$(add_pool_worktree "$case_dir" 1)
  app="$wt/packages/frontend"
  mkdir -p "$app/.next"
  printf '%s\n' '{"dependencies":{"next":"16"},"metadata":NaN}' \
    > "$app/package.json"
  printf 'build\n' > "$app/.next/BUILD_ID"
  printf 'packages/frontend/.next\n' >> "$wt/.gitignore"
  git -C "$wt" add -A >/dev/null 2>&1
  git -C "$wt" -c commit.gpgsign=false -c user.email=t@t -c user.name=t \
    commit -qm "nonstandard json constant"
  printf '1 available\n' > "$case_dir/pool-status"
  ...
  expect_code 1 "$rc" \
    "nonstandard-json-constant: package.json must use standard JSON"
  assert_present "$app/.next" \
    "nonstandard-json-constant: invalid JSON must refuse deletion"
  assert_contains "$out" "$wt" \
    "nonstandard-json-constant: the indeterminate copy must be named"
  pass "a non-standard JSON constant refuses the project"
}
...
SH
  ...
  assert_present "$wt/packages/frontend/.next" \
    "human-size-without-awk: report-only inspection must preserve build output"
  assert_contains "$out" "2.0M" \
    "human-size-without-awk: a formatter failure must not erase the reported size"
  pass "human-readable report sizes do not depend on an unchecked formatter"
}

test_sweep_never_invokes_removal() {
  local case_dir wt out rc real_rm
  case_dir=$(make_case no-removal)
  install_treehouse_stub "$case_dir"
  wt=$(add_pool_worktree "$case_dir" 1)
  add_next_app "$wt" packages/frontend
  printf '1 available\n' > "$case_dir/pool-status"
...
SH
  ...
  expect_code 0 "$rc" "no-removal: an unreachable removal failure must not affect reporting"
  assert_present "$wt/packages/frontend/.next" \
    "no-removal: the sweep must not invoke removal for build output"
  assert_contains "$out" "report-only" \
    "no-removal: the retained cache must still be reported"
  assert_not_contains "$out" "sweep: failed" \
    "no-removal: removal cannot become a sweep outcome"
  pass "the sweep has no path that invokes build-output removal"
}
...
test_sweep_accepts_recognized_next_dependency_tables() {
  local field case_dir wt app out rc
  for field in dependencies devDependencies peerDependencies optionalDependencies; do
    case_dir=$(make_case "next-$field")
    install_treehouse_stub "$case_dir"
    wt=$(add_pool_worktree "$case_dir" 1)
    app="$wt/packages/frontend"
    mkdir -p "$app/.next"
    printf '{"name":"app","%s":{"next":"16.3.0"}}\n' "$field" > "$app/package.json"
    printf 'build\n' > "$app/.next/BUILD_ID"
    printf 'packages/frontend/.next\n' >> "$wt/.gitignore"
    git -C "$wt" add -A >/dev/null 2>&1
    git -C "$wt" -c commit.gpgsign=false -c user.email=t@t -c user.name=t \
      commit -qm "next dependency in $field"
    printf '1 available\n' > "$case_dir/pool-status"
    ...
    expect_code 0 "$rc" "next-$field: recognized dependency evidence should succeed"
    assert_present "$app/.next" \
      "next-$field: recognized dependency evidence must not grant sweep deletion"
    assert_contains "$out" "report-only" \
      "next-$field: recognized build output should be reported without deletion"
  done
  pass "recognized dependency tables establish a Next.js app root"
}

test_sweep_leaves_ignored_next_with_stray_package_key() {
  local case_dir wt app out rc
  case_dir=$(make_case stray-package-next-key)
  install_treehouse_stub "$case_dir"
  wt=$(add_pool_worktree "$case_dir" 1)
  app="$wt/packages/frontend"
  mkdir -p "$app/.next"
  printf '{"name":"not-next","metadata":{"next":true}}\n' > "$app/package.json"
  printf 'irreplaceable\n' > "$app/.next/keep.txt"
  printf 'packages/frontend/.next\n' >> "$wt/.gitignore"
  git -C "$wt" add -A >/dev/null 2>&1
  git -C "$wt" -c commit.gpgsign=false -c user.email=t@t -c user.name=t \
    commit -qm "stray package next key"
  printf '1 available\n' > "$case_dir/pool-status"
  ...
  expect_code 0 "$rc" "stray-package-next-key: valid non-Next package should be determinate"
  assert_present "$app/.next/keep.txt" \
    "stray-package-next-key: an unrelated next key must not authorize deletion"
  assert_contains "$out" "nothing to reclaim" \
    "stray-package-next-key: no build output should be reported"
  pass "a stray package.json next key is not Next.js dependency evidence"
}
...
test_sweep_leaves_node_modules_and_source() {
  ...
  assert_present "$wt/packages/frontend/.next" \
    "node-modules: report-only inspection must preserve the app output"
...
test_sweep_reports_nested_build_output_once() {
  local case_dir wt out
  case_dir=$(make_case nested)
  install_treehouse_stub "$case_dir"
  wt=$(add_pool_worktree "$case_dir" 1)
  add_next_app "$wt" packages/frontend
  # Next's standalone output nests a second .next inside the first. The parent
  # must be reported once without walking into and reporting the nested copy.
  ...
  assert_present "$wt/packages/frontend/.next" \
    "nested: report-only inspection must preserve the whole tree"
  [ "$(printf '%s\n' "$out" | grep -Fc "$wt/packages/frontend/.next")" = 1 ] \
    || fail "nested: a nested .next must be reported with its parent exactly once"$'\n'"$out"
  pass "a nested standalone .next is reported with its parent once"
}
...
test_sweep_never_sweeps_the_project_clone() {
  ...
  pass "the sweep reports pooled copies only, never the project clone"
}

# --- teardown: build output is preserved on the way back to the pool ---------
...
test_teardown_preserves_build_output_when_returning_the_copy() {
  local case_dir out rc
  case_dir=$(make_teardown_case teardown-report-only)
  ...
  expect_code 0 "$rc" "teardown-report-only: teardown should succeed"
  assert_present "$case_dir/wt/packages/frontend/.next" \
    "teardown-report-only: teardown has no build-output deletion authority"
  assert_not_contains "$out" "reclaimed" \
    "teardown-report-only: teardown must not report a reclaim"
  pass "teardown preserves build output when returning the copy"
}

test_teardown_preserves_build_output_after_reaping_processes() {
  ...
  # The process reaper remains part of ordinary teardown even though cache
  # reclamation is absent. This stub records both facts at the pool return.
  cat > "$case_dir/fakebin/treehouse" <<SH
#!/usr/bin/env bash
...
SH
  ...
  # A live process whose working directory is the copy is exactly what
  # teardown's existing reaper clears before pool return.
  ...
  assert_grep "build-output-still-present" "$order" \
    "teardown-order: teardown must preserve build output before pool return"
  assert_grep "worktree-process-already-reaped" "$order" \
    "teardown-order: the existing process reaper must remain unchanged"
  pass "teardown preserves build output after reaping worktree processes"
}

test_teardown_preserves_build_output_without_lsof() {
  local case_dir out rc
  case_dir=$(make_teardown_case teardown-unproven-quietness)
  add_next_app "$case_dir/wt" packages/frontend
  git -C "$case_dir/wt" push -q origin fm/task-x1
  git -C "$case_dir/project" fetch -q origin
  ...
  out=$(FM_ROOT_OVERRIDE="$ROOT" \
    ...
    PATH="$case_dir/fakebin:/usr/bin:/bin" \
      "$TEARDOWN" task-x1 2>&1); rc=$?
  set -e

  expect_code 0 "$rc" "teardown-unproven-quietness: teardown should remain best effort"
  assert_present "$case_dir/wt/packages/frontend/.next" \
    "teardown-unproven-quietness: teardown must always preserve build output"
  assert_absent "$case_dir/state/task-x1.meta" \
    "teardown-unproven-quietness: existing best-effort teardown must continue"
  assert_not_contains "$out" "build-output reclamation" \
    "teardown-unproven-quietness: dead reclamation machinery must be absent"
  pass "teardown preserves build output when lsof is unavailable"
}
...
test_teardown_refusal_keeps_the_copy_intact() {
  ...
  pass "a refused teardown leaves build output intact"
}
...
test_teardown_stays_quiet_without_build_output() {
  ...
  pass "teardown has no build-output reclamation output"
}
...
run_next_cache_test test_sweep_never_invokes_removal
...
run_next_cache_test test_sweep_accepts_recognized_next_dependency_tables
run_next_cache_test test_sweep_leaves_ignored_next_with_stray_package_key
run_next_cache_test test_sweep_leaves_node_modules_and_source
run_next_cache_test test_sweep_reports_nested_build_output_once
run_next_cache_test test_sweep_never_sweeps_the_project_clone
run_next_cache_test test_teardown_preserves_build_output_when_returning_the_copy
run_next_cache_test test_teardown_preserves_build_output_after_reaping_processes
run_next_cache_test test_teardown_preserves_build_output_without_lsof
