...
# assert_pi_exit_guard <composer-state>: the exact empty-only gate bin/fm-control.sh
# do_exit uses before typing /quit. Unknown is a refusal, not a proceed.
assert_pi_exit_guard() {
  case "$1" in
    empty) return 0 ;;
    *) fail "fm-control exit would refuse composer_state='$1' (needs empty)" ;;
  esac
}
...
test_composer_state_pi_dollar_status_footer_is_empty() {
  ...
  assert_pi_exit_guard "$out"
  pass "fm_backend_herdr_composer_state: a dollar-first Pi status footer stays empty so fm-control exit proceeds"
}

test_composer_state_pi_blank_idle_composer_is_empty() {
  # 2026-09-18 local mates: idle Pi composer blank between separator rules, no
  # glyph. Identity plus the pair must prove empty without waiting on an
  # interrupt redraw.
  local dir log resp fb out
  dir="$TMP_ROOT/composer-pi-blank-idle"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf 'transcript\n─────────────────────────────────────────────────────\n\n─────────────────────────────────────────────────────\nproj (main)\n' > "$resp/1.out"
  printf '{"result":{"agent":{"agent":"pi","agent_status":"idle"}}}\n' > "$resp/2.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_composer_state lab:w1:p2' "$ROOT" )
  [ "$out" = empty ] || fail "a blank idle Pi separator composer should read empty, got '$out'"
  assert_pi_exit_guard "$out"
  pass "fm_backend_herdr_composer_state: a blank idle Pi composer stays empty so fm-control exit proceeds"
}
