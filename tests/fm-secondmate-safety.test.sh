...
# validation, treehouse lease handling, clone safety, child-worktree protection,
# and backlog-handoff safety. The happy-path
...
test_fm_home_parameterization() {
  ...
  if FM_HOME="$home_two" "$ROOT/bin/fm-project-mode.sh" app >/dev/null 2>&1; then
    fail "fm-project-mode invented a posture for a different home's missing registry"
  fi
...
test_home_seed_never_invokes_retired_pipeline_tool() {
  local home subhome fakebin log origin
  home="$TMP_ROOT/no-retired-tool-home"
  subhome="$TMP_ROOT/no-retired-tool-subhome"
  log="$TMP_ROOT/no-retired-tool.log"
  mkdir -p "$home/projects" "$home/data" "$home/state"
  fm_git_init_commit "$home/projects/alpha"
  fm_git_init_commit "$home/projects/beta"
  fm_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/no-retired-alpha.git"
  fm_git_add_origin "$home/projects/beta" "$TMP_ROOT/remotes/no-retired-beta.git"
  git clone --quiet "$ROOT" "$subhome"
  mkdir -p "$subhome/projects"
  origin=$(git -C "$home/projects/alpha" remote get-url origin)
  git clone --quiet "$origin" "$subhome/projects/alpha"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' '- beta [direct-PR] - beta project (added 2026-06-22)' > "$home/data/projects.md"
  fakebin=$(fm_fakebin "$TMP_ROOT/no-retired-tool-fake")
  cat > "$fakebin/no-mistakes" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$FM_FAKE_NO_MISTAKES_LOG"
exit 99
SH
  ...
  PATH="$fakebin:$PATH" FM_FAKE_NO_MISTAKES_LOG="$log" \
    FM_HOME="$home" FM_SECONDMATE_CHARTER='direct PR clone scope' FM_SECONDMATE_SCOPE='direct PR clone scope' \
    "$ROOT/bin/fm-home-seed.sh" design "$subhome" alpha beta >/dev/null \
    || fail "direct-PR seed failed while retired tool was present"
  [ ! -s "$log" ] || fail "home seeding invoked the retired delivery tool"
  [ -d "$subhome/projects/beta/.git" ] || fail "direct-PR seed did not clone beta"
  pass "home seeding never invokes the retired delivery tool"
}
