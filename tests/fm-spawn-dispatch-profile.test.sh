...
test_claude_launch_carries_no_claude_config_dir() {
  local rec id out status launch
  id=profile-claude-cfgdir-z17
  rec=$(make_spawn_case profile-claude-cfgdir claude "$id")
  read_case_record "$rec"

  mkdir -p "$CASE_DIR/supervisor-account" "$HOME_DIR/config"
  printf '%s\n' "$CASE_DIR/supervisor-account" > "$HOME_DIR/config/claude-config-dir"
  out=$(CLAUDE_CONFIG_DIR="$CASE_DIR/supervisor-account" \
    run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  ...
  assert_contains "$launch" "env -u CLAUDE_CONFIG_DIR" \
    "a claude launch must unset CLAUDE_CONFIG_DIR"
  assert_not_contains "$launch" "CLAUDE_CONFIG_DIR=" \
    "a claude launch must not export CLAUDE_CONFIG_DIR"
  assert_not_contains "$launch" "$CASE_DIR/supervisor-account" \
    "a claude worker launched on an inherited or leftover Claude pin"
  pass "a claude launch carries no CLAUDE_CONFIG_DIR, including when the spawning environment or leftover pin names another account"
}

# A Claude launch runs without CLAUDE_CONFIG_DIR and without the environment
# credentials Claude ranks above the stored /login, so a key the caller or the
# destination shell carries cannot silently bill another account. Executing the
# launch proves the shell actually drops them.
...
SH
  ...
  result=$(env -i HOME="$CASE_DIR" PATH="$FAKEBIN_DIR:$PATH" CLAUDE_CONFIG_DIR="$CASE_DIR/elsewhere" \
    ...
    ANTHROPIC_BASE_URL=https://proxy.invalid /bin/sh -c "$launch") || fail "claude launch did not run"
  [ "$result" = "unset|unset|unset|unset|https://proxy.invalid" ] \
    || fail "claude launch kept CLAUDE_CONFIG_DIR or an outranking credential: $result"
  pass "a claude launch unsets CLAUDE_CONFIG_DIR and sheds the credentials that outrank the stored login"
}
...
# A pinned Pi runner never falls back to its vendor's default root: with no pin
...
# Claude is not pinned; a missing config/claude-config-dir does not refuse.
test_account_pin_missing_refuses() {
  ...
  for harness in pi pi-signed; do
    ...
    assert_contains "$out" "Pi launches from home $HOME_DIR require an account pin: create $HOME_DIR/config/pi-agent-dir" \
  ...
  pass "pi and pi-signed spawns refuse without an account pin, naming the file to create"
}
...
test_account_pin_preflight_refuses() {
  ...
  for harness in claude pi pi-signed; do
    ...
    case "$harness" in
      claude) assert_contains "$out" "Claude's default login is not usable (quota-axi auth: keychain=missing)" \
        "claude preflight refusal must name the default login and the check's answer" ;;
...
# The credential shed that opens every Claude launch is owned by
# bin/fm-account-pin-lib.sh and proven behaviorally by
...
claude_expected_launch() {  # <home> <id> <permission-flag>
  ...
  printf '%s' "env -u CLAUDE_CONFIG_DIR -u CLAUDE_CODE_USE_BEDROCK -u CLAUDE_CODE_USE_VERTEX -u CLAUDE_CODE_USE_FOUNDRY -u ANTHROPIC_AUTH_TOKEN -u ANTHROPIC_API_KEY -u CLAUDE_CODE_OAUTH_TOKEN -u ANTHROPIC_PROFILE -u ANTHROPIC_FEDERATION_RULE_ID env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude $flag --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' $CLAUDE_CONTROL_CHANNEL_FLAG \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$home/data/$id/launch-brief.md')\""
}
