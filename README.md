...
- **Open Sourcerer Monitor** - the main home slow-polls one bounded account-wide union of open PRs authored by you or review-requested from, assigned to, or materially involving you through the same event-driven supervision plane, including repositories where you lack maintainer authority, with no cron job, daemon, per-PR poller, or extra token.
  It compares exact heads, checks, conflicts, external reviews, inline replies, conversation comments, merge, and close; unchanged scans make no model call, while one body-free changed-PR event routes to the existing implementation owner or fitting project second mate without creating duplicate workers.
  Every new head of a firstmate-authored PR gets one private adversarial review whose supported findings return to the branch owner, and external feedback is verified before a fix and one evidence-backed reply in the original thread; it never publishes its own findings under the PR author's identity, never approves, never merges, and `bin/fm-pr-review.sh opt-out <pr-url>` hands one PR back to you.
  A foreign-authored PR can receive at most one repository-policy comment review, and firstmate never changes its branch.
